<?php
if ( ! class_exists( 'Ideal_Magazine_Grid_Posts_Widget' ) ) {
	/**
	 * Adds Ideal_Magazine_Grid_Posts_Widget Widget.
	 */
	class Ideal_Magazine_Grid_Posts_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$ideal_magazine_grid_posts_widget_ops = array(
				'classname'   => 'grid-posts ascendoor-widget style-1',
				'description' => __( 'Retrive Grid Posts Widgets', 'ideal-magazine' ),
			);
			parent::__construct(
				'ideal_magazine_grid_posts_widget',
				__( 'Ascendoor Grid Posts Widget', 'ideal-magazine' ),
				$ideal_magazine_grid_posts_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$grid_count        = isset( $instance['number'] ) ? absint( $instance['number'] ) : 4;
			...
			<div class="main-wrap">
				<?php if ( ! empty( $grid_title || $grid_button_label ) ) { ?>
					<div class="title-heading">
						...
						<?php if ( ! empty( $grid_button_label ) ) : ?>
		...
		public function form( $instance ) {
			...
			<p>
				...
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="4" value="<?php echo absint( $grid_count ); ?>" size="3" />
			</p>
			<p>
