<?php
if ( ! class_exists( 'Ideal_Magazine_List_Posts_Thumbnail_Widget' ) ) {
	/**
	 * Adds Ideal_Magazine_List_Posts_Thumbnail_Widget Widget.
	 */
	class Ideal_Magazine_List_Posts_Thumbnail_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$ideal_magazine_list_posts_thumbnail_widget_ops = array(
				'classname'   => 'thumbnail-list-post ascendoor-widget style-1',
				'description' => __( 'Retrive List Posts Thumbnail Widgets', 'ideal-magazine' ),
			);
			parent::__construct(
				'ideal_magazine_list_posts_thumbnail_widget',
				__( 'Ascendoor List Posts Thumbnail Widget', 'ideal-magazine' ),
				$ideal_magazine_list_posts_thumbnail_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$list_posts_count        = isset( $instance['number'] ) ? absint( $instance['number'] ) : 4;
			...
			<div class="main-wrap">
				<?php if ( ! empty( $list_posts_title || $list_posts_button_label ) ) { ?>
					<div class="title-heading">
						<?php
						echo $args['before_title'] . esc_html( $list_posts_title ) . $args['after_title'];
						?>
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1 | Max:4):', 'ideal-magazine' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="4" value="<?php echo absint( $list_posts_count ); ?>" size="3" />
			</p>
			<p>
